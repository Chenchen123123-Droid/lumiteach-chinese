...
function SentenceOrderGame({ text = '', isFullscreen, onToggleFullscreen }) {
  ...
  // 初始化卡片
  useEffect(() => {
    const newCards = createSentenceCards(effectiveText);
    ...
    return (
      <div className="text-tool-setup">
        <div className="text-tool-setup-card">
          ...
          <h1>句子排序游戏</h1>
          <p>输入一句或多句中文，系统会拆成卡片供学生重新排序。</p>
          <label htmlFor="sentence-source">句子内容</label>
          <textarea
            id="sentence-source"
            value={draftText}
            onChange={event => setDraftText(event.target.value)}
            placeholder="例如：我每天早上七点起床，然后坐公交车去学校。"
            rows={7}
          />
          <div className="text-tool-setup-actions">
            <button type="button" className="btn-secondary" onClick={() => setDraftText('我每天早上七点起床，然后坐公交车去学校。')}>加载示例</button>
            <button type="button" className="btn-primary" disabled={!draftText.trim()} onClick={() => setLocalText(draftText.trim())}>生成卡片</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className={`sentence-game ${isFullscreen ? 'fullscreen' : ''}`}>
      <div className="game-header">
        ...
        <div className="game-header-actions">
          {!text.trim() && <button className="btn-fullscreen" onClick={() => { setDraftText(localText); setLocalText(''); }}>修改句子</button>}
