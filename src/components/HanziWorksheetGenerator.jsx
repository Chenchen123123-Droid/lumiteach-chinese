...
function HanziWorksheetGenerator() {
  ...
  // 计算每页行数（A4竖版约可容纳约20-25行）
  const ROWS_PER_PAGE = 22;

  // 分页
  const paginateRows = useCallback((allRows) => {
    const pages = [];
  ...
  const pages = useMemo(() => paginateRows(rows), [paginateRows, rows]);
                ...
                <div className="worksheet-content">
                  {pageRows.map((row, rowIndex) => (
                    <div key={rowIndex} className="practice-row">
                      ...
                      <div
                        className="cells-row"
                        style={{ gridTemplateColumns: `repeat(${cellsPerRow}, 1fr)` }}
                      >
